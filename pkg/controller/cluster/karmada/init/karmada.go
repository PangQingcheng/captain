...
func InitKarmadaResources(configData []byte, dir, caBase64, systemNamespace string) error {
...
func InitKarmadaBootstrapToken(configData []byte) (string, error) {
	restConfig, err := clientcmd.RESTConfigFromKubeConfig(configData)
	if err != nil {
		return "", err
	}
	...
	bootstrapToken, err := utils.GenerateRandomBootstrapToken(&metav1.Duration{Duration: utils.DefaultTokenDuration}, "", utils.DefaultGroups, utils.DefaultUsages)
	if err != nil {
		return "", err
	}

	if err := utils.CreateNewToken(clientSet, bootstrapToken); err != nil {
		return "", err
	}
...
func createExtralResources(clientSet *kubernetes.Clientset, dir string) error {
	...
	if err := AllowBootstrapTokensToPostCSRs(clientSet); err != nil {
		return err
	}

	if err := AutoApproveKarmadaAgentBootstrapTokens(clientSet); err != nil {
		return err
	}

	if err := AutoApproveAgentCertificateRotation(clientSet); err != nil {
		return err
	}
